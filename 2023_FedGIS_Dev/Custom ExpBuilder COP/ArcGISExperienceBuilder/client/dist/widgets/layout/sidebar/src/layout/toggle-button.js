...
function getIcon(icon, iconSource) {
    if (iconSource === 1) {
...
export class SidebarController extends React.PureComponent {
    getStyle() {
        var _a;
      ...
      border: ${(_a = styleUtils.toCSSBorder(border)) !== null && _a !== void 0 ? _a : 'none'} !important;
    ...
    render() {
        ...
        return (jsx(Button, { className: 'sidebar-controller d-flex justify-content-center align-items-center', css: this.getStyle(), style: this.flipStyle(style), onClick: onClick, "aria-expanded": expanded },
            jsx(Icon, { className: 'icon', rotate: expanded ? 0 + rotate : 180 + rotate, icon: getIcon(icon, iconSource), size: iconSize, color: color.normal.icon.color })));
    }
}
