...
export default class GeneralSettings extends React.PureComponent {
    constructor(props) {
        ...
        this.componentDidMount = () => {
            ...
            if (!this.state.apiLoaded) {
                loadArcGISJSAPIModules([
                    ...
                    if (this.props.config && this.props.config.defaultPointSymbol) {
                        pointSymbol = this.jsonUtils.fromJSON(this.props.config.defaultPointSymbol);
                    }
                    else {
                        pointSymbol = this.jsonUtils.fromJSON(defaultSymbol);
                    }
                    this.setState({
                        apiLoaded: true,
                        defaultSymbol: pointSymbol
                    });
                });
            }
        };
        this.onZoomScaleInputChange = (value) => {
            this.setState({
                zoomScaleInput: value
            });
            this.props.onGeneralSettingsUpdated('zoomScale', value);
        };
        this.showSymbolChooser = () => {
            this.setState({
                isPanelOpened: true
            });
        };
        this.onPointSymbolChanged = (currentSymbol) => {
        ...
        this.onSymbolChange = (currentSymbol) => {
            this.updatedSymbol = currentSymbol;
        };
        this.onSymbolClick = () => {
            this.setState({
                defaultSymbol: this.updatedSymbol
            }, () => {
                var _a;
                this.props.onGeneralSettingsUpdated('defaultPointSymbol', (_a = this.updatedSymbol) === null || _a === void 0 ? void 0 : _a.toJSON());
            });
        ...
        let zoomScale = 50000;
        if (this.props.config && this.props.config.zoomScale) {
            zoomScale = this.props.config.zoomScale;
        }
        this.state = {
            apiLoaded: false,
    ...
    render() {
        return jsx("div", { style: { height: '100%', marginTop: '5px' } },
            jsx("div", { css: getGeneralSettingsStyle(this.props.theme) },
                jsx(SettingRow, { label: this.nls('locationSymbol'), flow: 'wrap' }, this.state.defaultSymbol &&
                    jsx(SymbolSelector, { jimuSymbolType: JimuSymbolType.Point, symbol: this.state.defaultSymbol, onPointSymbolChanged: this.onPointSymbolChanged.bind(this) })),
                jsx(SettingRow, { className: 'hint', label: this.nls('symbolLocationHint') }),
                jsx(SettingRow, { label: this.nls('zoomScale') },
                    jsx(Label, null, "1 :"),
                    jsx(NumericInput, { "aria-label": this.nls('zoomScale'), size: 'sm', className: 'zoomScaleInput', defaultValue: this.state.zoomScaleInput, showHandlers: false, onChange: this.onZoomScaleInputChange }))));
    }
}
