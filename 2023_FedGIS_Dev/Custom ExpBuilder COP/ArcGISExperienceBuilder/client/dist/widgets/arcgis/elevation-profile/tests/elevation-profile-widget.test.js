import { React, getAppStore, appActions, Immutable } from 'jimu-core';
...
const theme = Object.assign(Object.assign({}, mockTheme), { surfaces: [{ bg: '#fff' }, { bg: '#fff' }] });
describe('Validate availability of Draw and Select tool depending on available data sources and selectable line layers', () => {
    it('If the map is not selected through selector or None is selected then widget placeholder should be shown', function () {
        const { queryByTestId } = render(React.createElement(Widget, { theme: theme, config: commonConfig, layouts: {} }));
    ...
    it('If the map is selected through a selector and no web map/scene is found in the Map widget then only the Draw tool should be available.', function () {
        const useMapWidgetId = Immutable(['widget_1']);
        const config = Object.assign(Object.assign({}, commonConfig), { useMapWidget: true, activeDataSource: 'default' });
        const { queryByTestId, getByTestId } = render(React.createElement(Widget, { useMapWidgetIds: useMapWidgetId, theme: theme, config: config, layouts: {} }));
    ...
    it('Only show draw tool if there are no selectable lines in the map/scene.', function () {
        const useMapWidgetId = Immutable(['widget_1']);
        const config = Object.assign(Object.assign({}, commonConfig), { useMapWidget: true, activeDataSource: 'dataSource_1', configInfo: {
                dataSource_1: {
                    advanceOptions: true,
                    profileSettings: {
                        layers: []
        ...
        const { getByTestId } = render(React.createElement(Widget, { useMapWidgetIds: useMapWidgetId, theme: theme, config: config, layouts: {} }));
        expect(getByTestId('drawButton')).toHaveClass('front-section');
        expect(getByTestId('selectButton')).toHaveClass('hidden');
    });
});
//# sourceMappingURL=elevation-profile-widget.test.js.map