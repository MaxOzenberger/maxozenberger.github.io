...
jest.mock('jimu-for-builder', () => (Object.assign(Object.assign({}, jest.requireActual('jimu-for-builder')), { templateUtils: {
        processForTemplate: jest.fn()
    } })));
jest.mock('jimu-ui', () => {
    return Object.assign(Object.assign({}, jest.requireActual('jimu-ui')), { NumericInput: (props) => React.createElement("input", Object.assign({ className: 'jimu-input jimu-input-sm jimu-numeric-input jimu-numeric-input-input' }, props)) });
});
initGlobal();
...
describe('bookmark setting test', function () {
    ...
    beforeAll(() => {
        render = widgetRender(getAppStore(), fixedMockTheme);
    });
    afterAll(() => {
        render = null;
    });
    ...
    it.only('click textinput should not trigger the reset', () => {
        ...
        const Setting = wrapWidget(BookmarkSetting, { theme: fixedMockTheme, ref });
        const { getByTitle } = render(React.createElement(Setting, Object.assign({ widgetId: 'bookmarkSettingTest1' }, props)));
        ref.current.handleSelect = jest.fn();
        fireEvent.click(getByTitle('SettingTest-2'));
        expect(ref.current.handleSelect).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=bookmark-setting.test.js.map