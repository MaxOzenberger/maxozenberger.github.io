...
export const DrawToolsSelector = React.memo((props) => {
    ...
    const _createItemUI = () => {
        ...
        for (let i = 0, len = toolsItmes.length; i < len; i++) {
            ...
            elements.push(jsx("li", { className: 'd-flex item', key: 'key-' + i },
                jsx(item.icon, { className: 'icon mr-2' }),
                jsx(Label, { className: 'd-flex item-label justify-content-between flex-grow-1 text-break', "aria-label": item.des },
                    item.des,
                    jsx(Switch, { className: 'd-flex', "data-itemid": item.id, checked: isChecked, onChange: onItemChange, "aria-label": item.des }))));
        }
        return elements;
    };
    const onItemChange = () => {
    ...
    return (jsx("div", { ref: rootRef, css: getStyle() },
        jsx("ul", { className: 'items-option', role: 'group', "aria-label": props.title }, _createItemUI())));
});
//# sourceMappingURL=draw-tools-selector.js.map