...
export default class CurvePath {
    constructor(points, sceneView, vec3, vec3d, esriMathUtils) {
        // path optimization
        this.longLineProcessing = {
            threshold: 400,
            distance: 150
        };
        this.acuteAngleProcessing = {
            angle: 90,
            threshold: 300,
            distance: 100
        };
        this.geoArrayToCoords = (geoArray) => {
            const vecs = [];
    ...
    preProcessing(glGeos) {
        ...
        for (let ii = 0; ii < (glGeos.length - 1); ii++) {
            ...
            const isFirst = !!(ii === 0);
            const isLast = !!(glGeos.length - 1 === ii + 1);
            let insertedRes;
            // 1 basePoint
    ...
    _getInsertControlPointRes(distance, isAcuteAngle) {
    ...
    _getInsertedPoint(p0, p1, dir, distance) {
...
//# sourceMappingURL=curve-path.js.map