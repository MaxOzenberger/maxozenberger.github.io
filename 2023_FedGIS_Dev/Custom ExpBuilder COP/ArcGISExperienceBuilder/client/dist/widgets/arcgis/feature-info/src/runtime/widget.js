/** @jsx jsx */
import { React, jsx, MessageManager, DataRecordsSelectionChangeMessage, DataSourceStatus, getAppStore, appConfigUtils } from 'jimu-core';
...
import { FontSizeType } from '../config';
...
export default class Widget extends React.PureComponent {
    constructor(props) {
        ...
        //  }
        // }
        this.onSelectedRecordIdChanged = (index, dataSourceId) => {
        ...
        this.onUnselectedRecordIdChanged = (dataSourceId) => {
            var _a;
            if (((_a = this.dataSource) === null || _a === void 0 ? void 0 : _a.id) === dataSourceId) {
                MessageManager.getInstance().publishMessage(new DataRecordsSelectionChangeMessage(this.props.id, []));
            }
        };
        this.onDataSourceStatusChanged = (status, dataSourceLabel) => {
            this.setState({
                loadDataStatus: status,
                dataSourceLabel: dataSourceLabel
            });
        };
        this.onDataChanged = (dataSource, currentData, isFirstLoad) => {
    ...
    selectGraphic() {
        var _a;
        const record = (_a = this.currentData) === null || _a === void 0 ? void 0 : _a.record;
        if (record && this.dataSource) {
            ...
            if (!selectedRecordIds.includes(recordId)) {
                this.dataSource.queryById(recordId).then((record) => {
    ...
    render() {
        ...
        dataSourceContent = (jsx(DataLoader, { useDataSource: useDataSource, widgetId: this.props.id, index: this.state.currentDataIndex, limitGraphics: this.props.config.limitGraphics, maxGraphics: this.props.config.maxGraphics, onSelectedRecordIdChanged: this.onSelectedRecordIdChanged, onUnselectedRecordIdChanged: this.onUnselectedRecordIdChanged, onDataSourceStatusChanged: this.onDataSourceStatusChanged, onDataChanged: this.onDataChanged }));
        let content = null;
        if (this.props.config.useMapWidget ? !useMapWidget : !useDataSource) {
            content = (jsx("div", { className: 'widget-featureInfo' },
                jsx(WidgetPlaceholder, { icon: featureInfoIcon, message: this.props.intl.formatMessage({ id: '_widgetLabel', defaultMessage: defaultMessages._widgetLabel }), widgetId: this.props.id })));
            this.currentData = null;
        }
        else {
            let loadingContent = null;
            if (this.state.loadDataStatus === DataSourceStatus.Loading) {
                loadingContent = (jsx("div", { style: { position: 'absolute', left: '50%', top: '50%' }, className: 'jimu-secondary-loading' }));
            }
            let dataActionPlaceholder = null;
            let dataActionConten = null;
            const dataName = this.props.intl.formatMessage({ id: 'featureInfoDataActionLabel', defaultMessage: defaultMessages.featureInfoDataActionLabel }, { layer: ((_a = this.dataSource) === null || _a === void 0 ? void 0 : _a.getLabel()) || '' });
            ...
            if (this.currentData && this.currentData.count > 0 && enableDataAction) {
                dataActionPlaceholder = (jsx("div", { className: 'data-action-placeholder' }));
                dataActionConten = (jsx("div", { className: 'data-action-dropdown-content' },
                    jsx(DataActionDropDown, { widgetId: this.props.id, dataSet: { dataSource: this.dataSource, records: [this.currentData.record], name: dataName }, type: 'tertiary' })));
            }
            let navContent = null;
            if (this.currentData && this.currentData.count > 1) {
                const featureNumbers = this.props.intl.formatMessage({ id: 'featureNumbers', defaultMessage: defaultMessages.featureNumbers }, { index: this.currentData.index + 1, count: this.currentData.count });
                navContent = (jsx("div", { className: 'nav-section d-flex justify-content-center align-items-center' },
                    jsx(Button, { className: 'nav-btn', type: 'tertiary', size: 'sm', onClick: this.onPreGraphicBtnClick },
                        " ",
                        '<',
                        " "),
                    jsx("span", null,
                        ...
                        featureNumbers,
                        " "),
                    jsx(Button, { className: 'nav-btn', type: 'tertiary', size: 'sm', onClick: this.onNextGraphictBtnClick },
                        " ",
                        '>',
                        " ")));
            }
            ...
            if (this.currentData && (this.currentData.count === 1 ? enableDataAction : this.currentData.count > 1) && this.state.loadDataStatus !== DataSourceStatus.NotReady) {
                headerConten = (jsx("div", { className: 'header-section' },
            ...
            if (this.state.loadDataStatus === DataSourceStatus.NotReady) {
                const warningMessage = this.props.intl.formatMessage({ id: 'outputDataIsNotGenerated', defaultMessage: 'warning' }, { outputDsLabel: this.state.dataSourceLabel, sourceWidgetName: this.props.dataSourceWidgetLabel });
                featureInfoContent = (jsx("div", { className: 'widget-featureInfo' },
                    jsx(WidgetPlaceholder, { icon: featureInfoIcon, message: this.props.intl.formatMessage({ id: '_widgetLabel', defaultMessage: defaultMessages._widgetLabel }), widgetId: this.props.id }),
                    jsx(Icon, { className: 'warning-icon', icon: this.warningIcon, size: 26, title: warningMessage, currentColor: false })));
            }
            else if (this.currentData && this.dataSource) {
                featureInfoContent = (jsx("div", { style: { cursor: 'pointer' }, onClick: this.onCurrentFeatureClick },
                    jsx(FeatureInfo, { graphic: this.currentData.graphic, visibleElements: visibleElements, dataSource: this.dataSource })));
            }
            else {
                featureInfoContent = (jsx("div", { className: 'no-data-message p-4 font-weight-bold', dangerouslySetInnerHTML: {
        ...
        return (jsx("div", { css: getStyle(this.props.theme, this.props.config.styleType, this.getStyleConfig(), (_c = (_b = getAppStore().getState()) === null || _b === void 0 ? void 0 : _b.appContext) === null || _c === void 0 ? void 0 : _c.isRTL), className: 'jimu-widget' }, content));
    }
}
...
Widget.mapExtraStateProps = (state, ownProps) => {
    ...
    return {
        dataSourceWidgetLabel: (_b = (_a = state.appConfig) === null || _a === void 0 ? void 0 : _a.widgets[dataSourceWidgetId]) === null || _b === void 0 ? void 0 : _b.label
    };
};
