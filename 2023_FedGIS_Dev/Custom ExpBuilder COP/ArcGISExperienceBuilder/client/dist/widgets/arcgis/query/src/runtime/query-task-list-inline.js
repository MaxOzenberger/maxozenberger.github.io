/** @jsx jsx */
import { jsx, css, React, classNames, lodash } from 'jimu-core';
...
export function TaskListInline(props) {
    ...
    const handleItemWidthChange = React.useCallback((index, width) => {
    ...
    const handlePopperOpenedChange = React.useCallback((index, isOpen) => {
        if (isOpen) {
    ...
    return (jsx("div", { ref: containerRef, css: style, className: classNames('runtime-query__query-list h-100 d-flex', { 'flex-wrap': wrap, 'align-content-start': wrap }) },
        expandedArray.map((queryItem, index) => {
            const currentItem = Object.assign({}, DEFAULT_QUERY_ITEM, queryItem);
            return jsx(TaskListPopperWrapper, { id: index, key: currentItem.configId, icon: currentItem.icon, buttonType: 'default', label: currentItem.displayLabel ? currentItem.name : undefined, popperTitle: currentItem.name, onWidthChange: handleItemWidthChange, minSize: minSize, defaultSize: defaultSize, onOpenedChange: handlePopperOpenedChange, forceClose: openedPopper >= 0 && openedPopper !== index },
                jsx(QueryTask, { widgetId: widgetId, index: index, total: 1, queryItem: currentItem, wrappedInPopper: true, className: 'pb-3' }));
        }),
        nonExpandedArray.length > 0 && (jsx(TaskListPopperWrapper, { id: expandedArray.length + 1, icon: iconMap.iconMore, minSize: minSize, buttonType: 'default', popperTitle: widgetLabel, defaultSize: defaultSize, onOpenedChange: handlePopperOpenedChange, forceClose: openedPopper >= 0 && openedPopper !== expandedArray.length + 1 },
            jsx(QueryTaskList, { widgetId: widgetId, queryItems: nonExpandedArray, className: 'py-3' })))));
}
//# sourceMappingURL=query-task-list-inline.js.map