import {
  React, DataSourceManager, ExtentChangeMessage, DataSourceComponent, portalUrlUtils, SqlQueryParams,
  getAppStore, MutableStoreManager, ImmutableObject, dataSourceUtils, FeatureLayerDataSource, UseDataSource
} from 'jimu-core'
import { IMConfig, SceneQualityMode } from '../../config'
import { MapDataSource, DataSourceTypes, loadArcGISJSAPIModules, MapViewManager, JimuMapView, zoomToUtils, DefaultMapInfo } from 'jimu-arcgis'
...
export default class MapBase extends React.PureComponent<Props, State> {
  ...
  componentDidUpdate (prevProps: Props) {
    ...
    if (this.getDsJsonFromDsId(curDsId).type === DataSourceTypes.WebMap) {
      ...
      this.analysisMapView().then(() => {
        this.updateMapView(this.props.baseWidgetProps.config, prevProps.baseWidgetProps?.config)
    ...
    if (this.getDsJsonFromDsId(curDsId).type === DataSourceTypes.WebScene) {
      ...
      this.analysisSceneView().then(() => {
        this.updateSceneView(this.props.baseWidgetProps.config, prevProps.baseWidgetProps?.config)
  ...
  updateMapView = (config: IMConfig, preConfig?: IMConfig): void => {
    const jimuMapViewId = `${this.props.baseWidgetProps.id}-${this.props.dataSourceId}`
    ...
    if ((config.selectionHighlightColor && config.selectionHighlightColor !== preConfig?.selectionHighlightColor) ||
        (config.selectionHighlightHaloColor && config.selectionHighlightHaloColor !== preConfig?.selectionHighlightHaloColor)) {
      const highlightOptions = {
        ...this.mapView.highlightOptions,
        color: new this.Color(config.selectionHighlightColor),
        haloColor: new this.Color(config.selectionHighlightHaloColor)
      }
  ...
  updateSceneView = (config: IMConfig, preConfig?: IMConfig): void => {
    const jimuMapViewId = `${this.props.baseWidgetProps.id}-${this.props.dataSourceId}`
    ...
    if ((config.selectionHighlightColor && config.selectionHighlightColor !== preConfig?.selectionHighlightColor) ||
        (config.selectionHighlightHaloColor && config.selectionHighlightHaloColor !== preConfig?.selectionHighlightHaloColor)) {
      const highlightOptions = {
        ...this.sceneView.highlightOptions,
        color: new this.Color(config.selectionHighlightColor),
        haloColor: new this.Color(config.selectionHighlightHaloColor)
      }
  ...
  queryExtentFromQueryParams = (mapBaseView: __esri.MapView | __esri.SceneView, dataSourceId: string, useDataSources: UseDataSource[]) => {
  ...
  handleAction = (mutableStateProps: ActionRelatedProps, mapBaseView: __esri.MapView | __esri.SceneView) => {
    if (mutableStateProps.zoomToFeatureActionValue) {
      if (mutableStateProps.zoomToFeatureActionValue.relatedWidgets &&
        ...
        const zoomToFeatureValue: any = mutableStateProps.zoomToFeatureActionValue.value
        let layer = null
    ...
    if (mutableStateProps.panToActionValue) {
      if (mutableStateProps.panToActionValue.relatedWidgets &&
      ...
      } else if ((mutableStateProps.panToActionValue?.value as any)?.type === 'pan-to-query-params') {
        ...
        this.queryExtentFromQueryParams(mapBaseView, panToValue.dataSourceId, panToValue.useDataSources).then((result) => {
          ...
          if (result.useDefaultExtentOfCurrentView) {
            ...
            this.panToGeometry([result?.extent], mapBaseView, relatedWidgets)
          }
        })
      } else {
        const panToValue = mutableStateProps.panToActionValue.value as any
  ...
  panToGeometry = (geometrys, mapBaseView, relatedWidgets) => {
    //const tempMapBaseView = mapBaseView as any
