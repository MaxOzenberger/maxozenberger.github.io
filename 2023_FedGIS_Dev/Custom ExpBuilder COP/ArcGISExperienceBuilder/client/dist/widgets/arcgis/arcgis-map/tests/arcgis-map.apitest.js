import { React, Immutable, DataSourceManager, getAppStore, appActions } from 'jimu-core';
...
describe('arcgis map widget', () => {
    ...
    describe('when a config with datasource is passed', () => {
        let wrapper;
        const config = {};
        const useDataSources = Immutable([{ dataSourceId: 'test-3c2519a766fc4592aad8cfe3595cc46e' }]);
        beforeAll(() => {
            const Widget = wrapWidget(MapWidget, { config, useDataSources, id: 'widget_1', dispatch: store.dispatch, queryObject: Immutable({}) });
            wrapper = mount(React.createElement(Provider, { store: store },
                React.createElement(Widget, { widgetId: 'widget_1' })));
        });
...
//# sourceMappingURL=arcgis-map.apitest.js.map