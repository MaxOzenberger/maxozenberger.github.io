...
/** @jsx jsx */
import { React, jsx, utils, moduleLoader, lodash, ReactResizeDetector } from 'jimu-core';
import { DisplayOrderType, ElevationUnitType, WidgetStyleType } from '../config';
import defaultMessages from './translations/default';
import { JimuMapViewComponent } from 'jimu-arcgis';
import { Button, Card, CardBody, CardFooter, Dropdown, DropdownButton, DropdownItem, DropdownMenu, hooks, WidgetPlaceholder, defaultMessages as jimuiDefaultMessages } from 'jimu-ui';
...
import { degToDDM, degToDMS, getCSUnitByCrs, getUnitRate, getUnits, isGeographicUnit, isProjectUnit, localizeNumberBySettingInfo } from '../utils';
...
const Widget = (props) => {
    ...
    const onActiveViewChange = (jimuMapView) => {
        ...
        if (enableRealtime) {
            ...
            (_e = clickListener.current) === null || _e === void 0 ? void 0 : _e.remove();
            (_f = moveListener.current) === null || _f === void 0 ? void 0 : _f.remove();
            clickListener.current = view === null || view === void 0 ? void 0 : view.on('click', (event) => {
                var _a, _b;
                const threeDPoint = { x: (_a = event === null || event === void 0 ? void 0 : event.native) === null || _a === void 0 ? void 0 : _a.pageX, y: (_b = event === null || event === void 0 ? void 0 : event.native) === null || _b === void 0 ? void 0 : _b.pageY };
                onMapClick(event, viewTypeIsThree ? threeDPoint : undefined);
        ...
        mapWkid.current = (_g = view === null || view === void 0 ? void 0 : view.spatialReference) === null || _g === void 0 ? void 0 : _g.wkid;
        mapPortalId.current = (_j = (_h = view === null || view === void 0 ? void 0 : view.map) === null || _h === void 0 ? void 0 : _h.portalItem) === null || _j === void 0 ? void 0 : _j.id;
    };
    const debounceOnResize = lodash.debounce((width, height) => onResize(width, height), 200);
    ...
    const projectMapPoint = (point, selectedSystem, threeDPoint) => {
        ...
        geometryService.project(defUrl, params).then(geometries => {
            ...
            if (outputUnit === 'MGRS' || outputUnit === 'USNG') {
                ...
                displayDegOrDms(outputUnit, point, mapSr.isWebMercator);
            }
            else {
                displayProject(outputUnit, point, mapSr.isWebMercator);
            }
        });
        const view = currentJimuMapView === null || currentJimuMapView === void 0 ? void 0 : currentJimuMapView.view;
        const viewTypeIsThree = (view === null || view === void 0 ? void 0 : view.type) === '3d';
    ...
    const displayDegOrDms = (unit, normalizedPoint, mapIsMercator) => {
        let { x, y } = normalizedPoint;
        const orderXy = displayOrder === DisplayOrderType.xy;
        // get unitRate
        const selectedSystem = coordinateSystem.find(item => item.id === selectedSystemId);
        const { crs } = selectedSystem;
        const unitRate = getUnitRate(getCSUnitByCrs(crs), unit, mapIsMercator);
        x = x * unitRate;
        y = y * unitRate;
    ...
    const displayProject = (unit, normalizedPoint, mapIsMercator) => {
        let { x, y } = normalizedPoint;
        // get unitRate
        const selectedSystem = coordinateSystem.find(item => item.id === selectedSystemId);
        const { crs } = selectedSystem;
    ...
    if (!useMap) {
        return (jsx(WidgetPlaceholder, { widgetId: id, icon: coordinatesIcon, "data-testid": 'coordinatesPlaceholder', message: placeHolderName }));
    }
    ...
    const modernCoorInfo = (showTips
        ...
        : (showMouseTips
            ? mouseMoveTips
            : jsx("div", { className: 'info-container' },
                jsx("div", { className: `d-flex w-100 ${hasElevOrEye ? 'h-50' : 'h-100'}` }, (geoInfo === computing || !geoInfo)
                    ? jsx("div", { className: 'coordinates-computing' }, geoInfo || ((elevNum || eyeNum) ? '--' : enableClickTips))
                    : jsx(TextAutoFit, { className: 'coordinates-card-text-geo', text: geoInfo, widgetRect: widgetRect, domChange: hasElevOrEye })),
                hasElevOrEye &&
                    jsx("div", { className: 'h-50' },
                        elevNum
                            ? jsx("div", { className: 'textfit-container' },
                                jsx(TextAutoFit, { className: 'coordinates-card-text', text: elevNum, widgetRect: widgetRect }),
                                jsx("div", { className: 'info-unit' }, translate('elev', { ele: unitAbbrMap[elevUnit] })))
                            : jsx("div", { className: 'textfit-container' },
                                jsx("div", { className: 'coordinates-card-text-empty' }, "--"),
                                jsx("div", { className: 'info-unit' }, translate('elev', { ele: unitAbbrMap[elevUnit] }))),
                        eyeNum
                            ? jsx("div", { className: 'textfit-container' },
                                jsx(TextAutoFit, { className: 'coordinates-card-text', text: eyeNum, widgetRect: widgetRect }),
                                jsx("div", { className: 'info-unit' }, translate('eyeAlt', { alt: unitAbbrMap[eyeUnit] })))
                            : jsx("div", { className: 'textfit-container' },
                                jsx("div", { className: 'coordinates-card-text-empty' }, "--"),
                                jsx("div", { className: 'info-unit' }, translate('eyeAlt', { alt: unitAbbrMap[eyeUnit] })))))));
    ...
    // Renderer
    return (jsx("div", { className: 'jimu-widget-coordinates jimu-widget h-100', css: getStyle(theme, isClassic, widgetRect) },
        isClassic
            ? jsx("div", { className: 'coordinates-widget-container d-flex justify-content-between' },
                jsx(Button, { icon: true, size: 'sm', type: 'tertiary', onClick: onLocateClick, active: locateActive, title: locateBtnTips, "aria-label": locateBtnTips, className: 'jimu-outline-inside coordinates-locate' },
                    jsx(LocatorOutlined, null)),
                jsx("div", { className: 'coordinates-info text-truncate', title: classicCoorInfo }, classicCoorInfo),
                jsx(CopyBtn, { text: classicCoorInfo, disabled: classicCopyDisable }),
                (coordinateSystem === null || coordinateSystem === void 0 ? void 0 : coordinateSystem.length) > 0 &&
                    jsx(Dropdown, { size: 'sm', activeIcon: true },
                        jsx(DropdownButton, { arrow: false, icon: true, size: 'sm', type: 'tertiary', className: 'suspension-drop-btn jimu-outline-inside', title: changeSystem },
                            jsx(DownOutlined, null)),
                        jsx(DropdownMenu, null, coordinateSystem.map(item => {
                            const isActived = item.id === selectedSystemId;
                            return (jsx(DropdownItem, { key: item.id, active: isActived, onClick: () => handleSystemChange(item.id) }, item.name));
                        }))))
            : jsx("div", { className: 'coordinates-widget-container w-100 h-100' },
                jsx(Card, { className: 'h-100 coordinates-card' },
                    jsx(CardBody, { className: 'widget-card-content' }, modernCoorInfo),
                    jsx(CardFooter, { className: 'widget-card-footer' },
                        jsx("div", { className: 'jimu-widget d-flex justify-content-between' },
                            jsx(Button, { icon: true, size: 'sm', type: 'tertiary', onClick: onLocateClick, active: locateActive, title: locateBtnTips, "aria-label": locateBtnTips },
                                jsx(LocatorOutlined, null)),
                            jsx("div", { className: 'd-flex justify-content-between' },
                                classicCoorInfo &&
                                    jsx(CopyBtn, { text: classicCoorInfo, disabled: modernCopyDisable, className: 'coordinates-card-copy mr-1' }),
                                jsx(Dropdown, { size: 'sm', activeIcon: true },
                                    jsx(DropdownButton, { arrow: false, icon: true, size: 'sm', type: 'tertiary', className: 'suspension-drop-btn', title: changeSystem },
                                        jsx(DownOutlined, { className: 'suspension-drop-btn' })),
                                    jsx(DropdownMenu, null, coordinateSystem.map(item => {
                                        const isActived = item.id === selectedSystemId;
                                        return (jsx(DropdownItem, { key: item.id, active: isActived, onClick: () => handleSystemChange(item.id) }, item.name));
                                    })))))))),
        jsx(JimuMapViewComponent, { useMapWidgetId: useMapWidgetIds === null || useMapWidgetIds === void 0 ? void 0 : useMapWidgetIds[0], onActiveViewChange: onActiveViewChange }),
        jsx(ReactResizeDetector, { handleWidth: true, handleHeight: true, onResize: debounceOnResize })));
};
