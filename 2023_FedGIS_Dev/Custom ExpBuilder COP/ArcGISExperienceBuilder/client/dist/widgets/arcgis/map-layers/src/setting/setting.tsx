...
import {
  ...
  UseDataSource, AllDataSourceTypes
} from 'jimu-core'
    ...
    if (this.state.useMapWidget) {
      ...
      actionsContent = (
        <React.Fragment>
          ...
          <SettingRow label={<FormattedMessage id='showOrHideLabels' defaultMessage={defaultMessages.showOrHideLabels} />}>
            <Switch
              className='can-x-switch' checked={(this.props.config && this.props.config.label) || false}
              data-key='goto' onChange={evt => { this.onToolsChanged(evt.target.checked, 'label') }}
            />
          </SettingRow>
          <SettingRow label={<FormattedMessage id='transparency' defaultMessage={defaultMessages.layerTransparency} />}>
            <Switch
