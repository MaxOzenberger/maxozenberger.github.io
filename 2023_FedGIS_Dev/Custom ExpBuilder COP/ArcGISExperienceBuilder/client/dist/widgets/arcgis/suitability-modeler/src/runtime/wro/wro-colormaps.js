...
let colormapDefinitions;
let labelsInitialized = false;
export function findColormapInfo(name) {
    let found;
    if (colormapDefinitions) {
        colormapDefinitions.some((def) => {
            if (name === def.name) {
                found = def;
            }
            return !!found;
...
export function findColormapNameFromRasterColormap(colormap) {
    let found;
    if (colormapDefinitions) {
        colormapDefinitions.some((def) => {
            const match = def.colors.every((colorInfo, index) => {
                let match2 = false;
...
export function getDefaultColormapName() {
    return '1_9_green_yellow_red';
}
export function initLabels(nls) {
...
function makeColormapDefinitions() {
    const definitions = [
        {
...
//# sourceMappingURL=wro-colormaps.js.map