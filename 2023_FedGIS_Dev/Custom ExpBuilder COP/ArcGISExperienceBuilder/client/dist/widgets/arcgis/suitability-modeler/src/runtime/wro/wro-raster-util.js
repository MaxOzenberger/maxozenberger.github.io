...
function createColormapParams(options) {
    if (!options)
        options = {};
    const args = {
...
function createLocalParams(operation, options) {
    if (!options) {
        options = {};
    }
    const args = {
...
function createRemapParams(options) {
    if (!options)
        options = {};
    const args = {
        Raster: options.Raster || '$$',
...
export function createRemapRanges(inputRanges, outputValues, noDataRanges, options) {
    const remapRanges = [];
    let hasValidLabels = false;
    let labels, noDataLabels, sortBy;
    if (options) {
...
function createWeightedSumParams(rasterInfos, colormap) {
...
function colormapInfoToColormap(colormapInfo) {
    return colormapInfo.colors.map(colorInfo => {
        return [colorInfo.value].concat(colorInfo.rgb);
    });
}
;
export function featureToRasterLayer(feature) {
    ...
    if (attr) {
        ...
        rasterLayer.remapRanges = createRemapRanges(inputRanges, outputValues, noDataRanges, {
            labels: labels,
...
export function modelToItemData(wroModel) {
...
export function modelToRasterFunctionJson(wroModel) {
    ...
    const cmapInfo = findColormapInfo(wroModel.colormapName);
    if (cmapInfo) {
...
function rasterFunctionArgumentsToRemapRanges(rasterFunctionArguments) {
    let remapRanges;
...
function remapRangesToRasterFunctionArguments(remapRanges) {
    if (!remapRanges)
        return;
...
export function renderingRuleToModel(wroModel, renderingRule) {
    ...
    if (nextFunction.rasterFunction === 'Local' &&
        ...
        const olyrs = rasterFunctionArguments.Rasters.map(multiplyFunction => {
            var _a;
            if ((_a = multiplyFunction === null || multiplyFunction === void 0 ? void 0 : multiplyFunction.rasterFunctionArguments) === null || _a === void 0 ? void 0 : _a.Rasters) {
                const olyr = {};
                multiplyFunction.rasterFunctionArguments.Rasters.forEach(raster => {
                    if (isNaN(raster)) {
                        ...
                        // get remap ranges
                        olyr.remapRanges = rasterFunctionArgumentsToRemapRanges(raster.rasterFunctionArguments);
                        if (rlyr === null || rlyr === void 0 ? void 0 : rlyr.remapRanges) {
                            rlyr.remapRanges.forEach((rr, i) => {
                                const rr2 = olyr.remapRanges[i];
                                if (rr2)
                                    rr2.label = rr.label;
                            });
                        }
        ...
        olyrs.forEach(olyr => {
            if (olyr) {
                if (typeof olyr.id === 'number' && typeof olyr.weight === 'number' &&
                    ...
                    const ml = ModelFactory.findModelLayer(wroModel, olyr.id);
                    if (ml) {
                        ml.selected = true;
...
//# sourceMappingURL=wro-raster-util.js.map