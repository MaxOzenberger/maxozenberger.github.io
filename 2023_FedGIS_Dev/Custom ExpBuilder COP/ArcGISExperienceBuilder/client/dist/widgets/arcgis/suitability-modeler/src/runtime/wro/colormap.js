...
export class Colormap extends React.PureComponent {
    render() {
      ...
      margin: 0 6px;
    `;
        return (jsx("div", { className: 'widget-wro-color-ramp' },
            jsx("span", { css: labelStyle }, lowLabel),
            jsx("span", { className: "widget-wro-color-ramp-container" }, colors),
            jsx("span", { css: labelStyle }, highLabel)));
    }
    renderColor(colorInfo) {
        ...
        return (jsx("span", { key: colorInfo.value.toString(), className: 'widget-wro-color-ramp-color', style: style, title: tip }));
    }
}
export class ColormapSelector extends React.PureComponent {
    render() {
        ...
        let cm;
        const colormapName = wroModel === null || wroModel === void 0 ? void 0 : wroModel.colormapName;
        if (colormapName) {
            const def = wroColormaps.findColormapInfo(colormapName);
            if (def) {
                cm = (jsx(Colormap, { defName: def.name, lowLabel: lowLabel, highLabel: highLabel, nls: this.props.wroContext.nls }));
            }
        }
        const items = [];
        const colormapDefinitions = wroColormaps.getDefinitions();
        if (wroModel && colormapDefinitions) {
            colormapDefinitions.forEach(def => {
                items.push(jsx(DropdownItem, { key: def.name, onClick: () => {
                        modelController.setColormapName(wroModel, def.name);
                    } },
                    jsx(Colormap, { defName: def.name, lowLabel: lowLabel, highLabel: highLabel, nls: this.props.wroContext.nls })));
            });
        }
        return (jsx("div", { className: 'widget-wro-colormap-selector' },
            jsx(Dropdown, { title: nls('wro_main_setSymbology') },
                jsx(DropdownButton, null, cm),
                jsx(DropdownMenu, { className: 'widget-wro-color-dropdown-menu' }, items))));
    }
}
