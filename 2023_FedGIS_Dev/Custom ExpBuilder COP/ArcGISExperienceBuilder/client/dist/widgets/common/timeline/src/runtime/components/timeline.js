/** @jsx jsx */
import { React, jsx, classNames, useIntl, dateUtils, defaultMessages as jimuCoreMessages, //, lodash
ReactRedux, AppMode, lodash, getAppStore } from 'jimu-core';
import { Button, Dropdown, DropdownButton, DropdownMenu, DropdownItem, defaultMessages as jimuUIMessages, Tooltip, hooks, Icon, Popper, Switch, Label } from 'jimu-ui';
import { getValueTickPairs, getStepEndTimeForTarget, bindResizeHandler, bindDragHandler, getValueTickInfoForYear, getValueTickInfoForMonth, getValueTickInfoForDay, getValueTickInfoForHour, getValueTickInfoForMinute, getTickSizes, getWidthByZoomLevel, getMaxWidthAndZoomLevel, getMultipleByZoomLevel } from './utils';
...
function TimeLine(props) {
    const { width: _width, height: _height, applied, timeStyle = TimeStyle.Classic, foregroundColor, backgroundColor, sliderColor, theme, startTime, endTime, accuracy = 'year', stepLength, dividedCount, cumulatively = false, enablePlayControl = false, speed: _speed = TimeSpeed.Medium, autoPlay: _autoPlay, updating = false, onTimeChanged, onApplyStateChanged } = props;
    ...
    React.useEffect(() => {
        ...
        return () => {
            var _a;
            (_a = containerRef.current) === null || _a === void 0 ? void 0 : _a.removeEventListener('mousedown', mouseDownHandler);
            interactableForResize === null || interactableForResize === void 0 ? void 0 : interactableForResize.unset();
            interactableForDrag === null || interactableForDrag === void 0 ? void 0 : interactableForDrag.unset();
        };
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);
    React.useEffect(() => {
     ...
     */
    const isAutoplaySuspendMode = ReactRedux.useSelector((state) => {
        var _a, _b;
        if (!autoPlay) {
            return null;
        }
        return ((_a = state.appRuntimeInfo) === null || _a === void 0 ? void 0 : _a.appMode) === AppMode.Design || ((_b = state.appRuntimeInfo) === null || _b === void 0 ? void 0 : _b.isPrintPreview);
    });
     ...
     */
    const updateLeftPositionByPlay = React.useCallback((newSTime = startTimeForStep, newETime = endTimeForStep, isNext) => {
     ...
     */
    const updateLeftPositionByZoom = React.useCallback((zoomIn) => {
    ...
    const timelineRulerByDIV = React.useMemo(() => {
        ...
        return jsx("div", { className: 'timeline-ticks' }, [years, months, days, hours, minutes].map((items, unitIndex) => {
            return items.map((item, index) => {
                const position = item.position;
                const unit = units[unitIndex];
                return jsx("div", { key: `item-${unitIndex}-${index}`, className: 'timeline-tick-container', "data-unit": unit, style: { left: position } },
                    item.label &&
                        jsx("div", { className: `timeline-tick_label ${tickSizes.labels[unit]}-label ${(unit === 'year' && index === 0 && leftPosition === 0) ? 'timeline-first_label' : ''}` }, item.label),
                    jsx("div", { key: index, className: classNames(`timeline-tick ${tickSizes.ticks[unit]}-tick`, item.label ? 'has-label' : 'no-label') }));
            });
        }));
        // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    const zoomOutButton = React.useMemo(() => {
        return jsx(Button, { icon: true, type: 'tertiary', size: 'sm', disabled: zoomLevel === 0, onClick: () => {
                updateLeftPositionByZoom(false);
                setZoomLevel(Math.max(0, zoomLevel - 1));
            } },
            jsx(MinusCircleOutlined, null));
    }, [zoomLevel, updateLeftPositionByZoom]);
    const zoomInButton = React.useMemo(() => {
        return jsx(Button, { icon: true, type: 'tertiary', size: 'sm', disabled: zoomLevel === (maxWidthAndZoomLevel === null || maxWidthAndZoomLevel === void 0 ? void 0 : maxWidthAndZoomLevel.zoomLevel), onClick: () => {
                updateLeftPositionByZoom(true);
                setZoomLevel(Math.min(maxWidthAndZoomLevel === null || maxWidthAndZoomLevel === void 0 ? void 0 : maxWidthAndZoomLevel.zoomLevel, zoomLevel + 1));
            } },
            jsx(PlusCircleOutlined, null));
    }, [zoomLevel, maxWidthAndZoomLevel, updateLeftPositionByZoom]);
    const playControlButton = React.useMemo(() => {
        if (!enablePlayControl) {
            return null;
        }
        return jsx(Tooltip, { title: messages(autoPlay ? 'pause' : 'play'), placement: 'bottom' },
            jsx(Button, { icon: true, type: 'tertiary', size: 'sm', className: 'play-btn', onClick: () => setAutoPlay(!autoPlay) }, autoPlay ? jsx(PauseOutlined, null) : jsx(PlayOutlined, null)));
    }, [enablePlayControl, autoPlay, messages]);
    const prevNextButtons = React.useMemo(() => {
        return jsx(React.Fragment, null,
            jsx(Tooltip, { title: messages('previous'), placement: 'bottom' },
                jsx(Button, { icon: true, type: 'tertiary', size: 'sm', onClick: e => onPreviousOrNextClick(false) },
                    jsx(PreviousOutlined, null))),
            timeStyle === TimeStyle.Classic && playControlButton,
            jsx(Tooltip, { title: messages('next'), placement: 'bottom' },
                jsx(Button, { icon: true, type: 'tertiary', size: 'sm', className: 'next-btn', onClick: e => onPreviousOrNextClick(true) },
                    jsx(NextOutlined, null))));
    ...
    const InfoButton = React.useMemo(() => {
        ...
        return jsx(React.Fragment, null,
            jsx(Button, { icon: true, type: 'tertiary', onClick: evt => setShowInfoPopper(!showInfoPopper), ref: ref => { infoIconRef.current = ref; } },
                jsx(InfoOutlined, null)),
            jsx(Popper, { open: showInfoPopper, keepMount: true, showArrow: true, reference: infoIconRef, toggle: (showInfoPopper) => {
                    setShowInfoPopper(!showInfoPopper);
                    lodash.defer(() => {
                        infoIconRef.current.focus();
                    });
                } },
                jsx("div", { className: 'p-3' },
                    jsx("h6", { className: 'mb-2' }, messages('overallTimeExtent')),
                    jsx("div", { className: 'mb-3' }, `${startLabel} - ${endLabel}`),
                    jsx(Label, { check: true, className: 'd-flex align-items-center' },
                        jsx("h6", { className: 'flex-grow-1 mb-0 mr-1' }, messages('filteringApplied')),
                        jsx(Switch, { checked: applied, onChange: (evt, checked) => {
    ...
    const SpeedDropdown = React.useMemo(() => {
        return jsx(Dropdown, { activeIcon: true },
            jsx(Tooltip, { title: messages('speed'), placement: 'bottom' },
                jsx(DropdownButton, { icon: true, type: 'tertiary', arrow: false, "aria-label": messages('speed'), "a11y-description": speedItems.filter(item => item.value === speed)[0].label },
                    jsx(SpeedOutlined, null))),
            jsx(DropdownMenu, null, speedItems.map(item => {
                return jsx(DropdownItem, { key: item.value, value: item.value, active: item.value === speed, onClick: evt => setSpeed(evt.target.value) }, item.label);
            })));
    }, [speedItems, speed, messages]);
    const changeNextScreen = hooks.useEventCallback((isNext) => {
        const extent = endTime - startTime;
        const multiple = getMultipleByZoomLevel(width, zoomLevel, maxWidthAndZoomLevel);
        const currentExtent = extent / multiple;
        const currentExtentStart = startTime + leftPosition / 100 * extent;
    ...
    const getMarginLeftRight = (_startTimeForStep, _endTimeForStep) => {
        let startPositionForStep = (_startTimeForStep - startTime) / (endTime - startTime);
    ...
    return (jsx("div", { css: timeStyles, dir: 'ltr', className: classNames('timeline w-100', {
            'timeline-classic': timeStyle === TimeStyle.Classic,
            'timeline-modern': timeStyle === TimeStyle.Modern
        }) },
        timeStyle === TimeStyle.Modern && jsx("div", { className: 'timeline-left' },
            jsx("div", { className: 'play-container' }, prevNextButtons)),
        jsx("div", { className: 'timeline-middle' },
            jsx("div", { className: 'timeline-header' },
                timeStyle === TimeStyle.Classic && jsx("div", { className: 'range-label', dir: isRTL ? 'rtl' : 'ltr' },
                    jsx("div", { className: 'range-label-badge' }),
                    startLabelForStep + ' - ' + endLabelForStep),
                timeStyle === TimeStyle.Modern && jsx(React.Fragment, null,
                    InfoButton,
                    jsx("div", { className: 'range-label', dir: isRTL ? 'rtl' : 'ltr' },
                        jsx("div", { className: 'range-label-badge' }),
                        startLabelForStep + ' - ' + endLabelForStep)),
                jsx("div", { className: 'zoom-container' },
                    zoomLevel === 0
                        ? zoomOutButton
                        : jsx(Tooltip, { title: messages('zoomOut'), placement: 'bottom' }, zoomOutButton),
                    zoomLevel === (maxWidthAndZoomLevel === null || maxWidthAndZoomLevel === void 0 ? void 0 : maxWidthAndZoomLevel.zoomLevel)
                        ? zoomInButton
                        : jsx(Tooltip, { title: messages('zoomIn'), placement: 'bottom' }, zoomInButton))),
            jsx("div", { className: 'timeline-content' },
                jsx("div", { className: 'timeline-content-inside' },
                    jsx("div", { className: 'timeline-whole', ref: el => (containerRef.current = el), style: { width: wMultiple * 100 + '%', height: height + 'px', marginLeft: -(leftPosition * wMultiple) + '%' } },
                        timeStyle === TimeStyle.Modern && jsx("div", { style: { height: (height - 32) + 'px' } }),
                        timelineRulerByDIV,
                        timeStyle === TimeStyle.Modern && jsx("div", { className: 'timeline-range-container', style: { height: height + 'px', marginTop: -(height - 32) + 'px' } },
                            jsx("div", { className: 'resize-handlers', ref: el => (resizeRef.current = el), style: { marginLeft: startPositionForStep, width: widthForStep * 100 + '%' } },
                                jsx("button", { className: `resize-handler resize-left ${_startTimeForStep === _endTimeForStep ? 'show-bg' : ''}`, ref: ref => { startRef.current = ref; }, title: startLabelForStep }),
                                jsx("button", { className: `resize-handler resize-right ${_startTimeForStep === _endTimeForStep ? 'show-bg' : ''}`, ref: ref => { endRef.current = ref; }, title: endLabelForStep }))),
                        showLeftArrow && jsx(Button, { icon: true, type: 'tertiary', size: 'sm', className: 'timeline-arrow left-arrow', style: { left: timeStyle === TimeStyle.Modern && !enablePlayControl && isRTL ? '25px' : 'null' }, onClick: evt => changeNextScreen(false) },
                            jsx(Icon, { width: 4, height: 16, icon: arrowIcon })),
                        showRightArrow && jsx(Button, { icon: true, type: 'tertiary', size: 'sm', className: 'timeline-arrow right-arrow', style: { right: timeStyle === TimeStyle.Modern && !enablePlayControl && !isRTL ? '25px' : 'null' }, onClick: evt => changeNextScreen(true) },
                            jsx(Icon, { width: 4, height: 16, icon: arrowIcon }))),
                    timeStyle === TimeStyle.Classic && jsx("div", { className: 'timeline-range-container', style: { width: wMultiple * 100 + '%', marginLeft: -(leftPosition * wMultiple) + '%' } },
                        jsx("div", { className: 'resize-handlers', ref: el => (resizeRef.current = el), style: { marginLeft: startPositionForStep, width: widthForStep * 100 + '%' } },
                            jsx("button", { className: 'resize-handler resize-left', ref: ref => { startRef.current = ref; }, title: startLabelForStep }),
                            jsx("button", { className: 'resize-handler resize-right', ref: ref => { endRef.current = ref; }, title: endLabelForStep }))))),
            timeStyle === TimeStyle.Classic && jsx("div", { className: 'timeline-footer' },
                InfoButton,
                jsx("div", { className: 'play-container' }, prevNextButtons),
                enablePlayControl ? SpeedDropdown : jsx("div", null))),
        timeStyle === TimeStyle.Modern && jsx("div", { className: 'timeline-right' },
            jsx("div", { className: 'play-container' },
                playControlButton,
