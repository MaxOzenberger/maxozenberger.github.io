...
export var TimelineAccuray;
(function (TimelineAccuray) {
    TimelineAccuray[TimelineAccuray["year"] = 1] = "year";
    TimelineAccuray[TimelineAccuray["month"] = 2] = "month";
    TimelineAccuray[TimelineAccuray["day"] = 3] = "day";
    TimelineAccuray[TimelineAccuray["hour"] = 4] = "hour";
    TimelineAccuray[TimelineAccuray["minute"] = 5] = "minute";
    TimelineAccuray[TimelineAccuray["second"] = 6] = "second";
})(TimelineAccuray || (TimelineAccuray = {}));
/**
 ...
 */
export function getValueTickPairs(props) {
    const { width, startTime, endTime, accuracy = 'hour' } = props;
 ...
 */
function getRenderedYearByCount(startTime, endTime, width) {
 ...
 * @returns ValueTickPair
 */
function getRenderedMonthByCount(yearTickWidth, width) {
 ...
 * @returns
 */
function getRenderedDayByCount(startTime, endTime, width) {
    const count = width / MINTICKWIDTH; // The total count
    let day;
 ...
 * @returns ValueTickPair
 */
function getRenderedHourByCount(tickWidth, width) {
 ...
 * @returns ValueTickPair
 */
function getRenderedMinuteOrSecondByCount(tickWidth, width) {
...
/************************** Ticks for divided *************************/
function getDateLabelForDividedTick(dateTime) {
 ...
 */
export function getValueTickInfoForDivided(startTime, endTime, dividedCount) {
    if (!dividedCount) {
        return null;
    }
    const ticks = [];
 ...
 */
export function getValueTickInfoForYear(details, width, localStartTime, localEndTime, leftPosition, endPosition, scale, marginForLR) {
...
// 10/01 2021
// getMonth()  = 9
export function getValueTickInfoForMonth(details, width, localStartTime, localEndTime, leftPosition, endPosition, scale) {
...
function skipToNextMonth(date, day, details) {
    let skip = false;
 ...
 */
export function getValueTickInfoForDay(details, width, localStartTime, localEndTime, leftPosition, endPosition, scale) {
    if (!details.day) {
        return [];
    }
    const startTime = new Date(localStartTime);
 ...
 */
export function getValueTickInfoForHour(details, width, localStartTime, localEndTime, leftPosition, endPosition, scale) {
    if (!details.hour) {
        return [];
    }
    const startTime = new Date(localStartTime);
 ...
 * Get all ticks' {value, label, position} for hour
 */
export function getValueTickInfoForMinute(details, width, localStartTime, localEndTime, leftPosition, endPosition, scale) {
    if (!details.minute) {
        return [];
    }
    const startTime = new Date(localStartTime);
...
export function bindDragHandler(interact, dragRef, getDragProps, resizeHandlerDragging, resizeHandlerDragend) {
    let dw;
    let dragProps;
...
function getDragingExtentValues(valueForDw, dragProps, initStartValue, initEndValue, startValue, endValue, newTempEnd) {
...
export function bindResizeHandler(interact, resizeRef, getResizeProps, resizeHandlerDragging, resizeHandlerDragend) {
    let dw;
    let initWidth;
    const edges = {
        left: '.resize-left',
...
function getDragValuesByAccuracy(event, clientWidth, resizeProps, initStartValue, initEndValue, startValue, endValue) {
 ...
 * @returns
 */
function getNewValueByAccuracy(accuracy, date, diff) {
    switch (accuracy) {
        case 'year':
...
export function getStepEndTimeForTarget(startTime, endTime, targetEndTime, stepLength, dividedCount, isNext = true) {
    ...
    if (dividedCount) {
        ...
        switch (stepLength === null || stepLength === void 0 ? void 0 : stepLength.unit) {
            case 'year':
 ...
 */
export function getTickSizes(tsDetails, years, months, days, hours, minutes) {
    const sizes = {
        labels: {},
        ticks: {}
    };
...
export function checkIfItemShown(leftPosition, endPosition, itemPosition, widthNum) {
    let isShown = false;
...
export function getMultipleByZoomLevel(width, zoomLevel, maxWidthAndZoomLevel) {
    if ((maxWidthAndZoomLevel === null || maxWidthAndZoomLevel === void 0 ? void 0 : maxWidthAndZoomLevel.zoomLevel) === zoomLevel && zoomLevel !== 0) {
        return maxWidthAndZoomLevel.maxWidth / width;
    }
    return Math.pow(2, zoomLevel);
}
export function getWidthByZoomLevel(width, zoomLevel, maxWidthAndZoomLevel) {
    return width * getMultipleByZoomLevel(width, zoomLevel, maxWidthAndZoomLevel);
}
const MAX_ZOOM_LEVEL = 30;
export function getMaxWidthAndZoomLevel(width, startTime, endTime, accuracy) {
