...
const Effect = (props): React.ReactElement => {
  ...
  return (
    <div css={style}>
      <Button aria-label={props['aria-label']} icon type='tertiary' size='sm' onClick={onClick} aria-pressed={active}>
...
export const Effects = (props: Props): React.ReactElement => {
  ...
  return (
    <div css={style} role='group'>
      <div className='line-one'>
        {EFFECTS1.map(([effect, icon], index) => {
          return <Effect aria-label={index === 0 ? translate('none') : translate('effect') + index} key={index} icon={icon} active={effect ? effect === value : !value} onClick={() => onChange(effect)} />
        })}
      </div>
      <div className='line-two'>
        {EFFECTS2.map(([effect, icon], index) => {
          return <Effect aria-label={translate('effect') + (5 + index)} key={index} icon={icon} active={effect === value} onClick={() => onChange(effect)} />
        })}
      </div>
    </div>
  )
