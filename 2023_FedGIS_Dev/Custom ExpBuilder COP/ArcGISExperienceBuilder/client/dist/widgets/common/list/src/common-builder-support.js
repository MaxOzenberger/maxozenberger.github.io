...
import { LayoutItemType, getAppStore, appActions } from 'jimu-core';
import { searchUtils } from 'jimu-layouts/layout-runtime';
import { Status } from './config';
export function selectSelf(props, fromPage) {
        ...
        if (layoutInfos.length > 1) {
            ...
            if (widgetId) {
                ...
                if (widgetJson &&
                    ...
                    const currentStatus = getAppStore().getState().widgetsState &&
...
export function handleResizeCard(props, newCardSize, widgetConfig, isTop = false, isLeft = false, isEnd = false, appConfig = undefined) {
    ...
    action.editWidgetConfig(id, widgetConfig
        .setIn(['cardConfigs', Status.Regular, 'cardSize', browserSizeMode], newCardSize)
