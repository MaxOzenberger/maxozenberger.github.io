  ...
  return <div className="widget-setting-navigator jimu-widget-setting" css={style}>
      ...
      {section && <React.Fragment>
        ...
        {(type === 'nav' && showIcon) && <React.Fragment>
          ...
          <SettingRow flow="no-wrap" label={translate('currentView')} >
            <NavIconPicker configurableOption={'none'} hideRemove size={8} icon={activedIcon as any} onChange={(icon) => onSettingConfigChange(['display', 'standard', 'activedIcon'], icon)}></NavIconPicker>
          </SettingRow>
          <SettingRow flow="no-wrap" label={translate('others')} >
            <NavIconPicker configurableOption={'none'} hideRemove size={8} icon={alternateIcon as any} onChange={(icon) => onSettingConfigChange(['display', 'standard', 'alternateIcon'], icon)}></NavIconPicker>
          </SettingRow>
        </React.Fragment>
        }

        {
          type === 'navButtonGroup' && <SettingRow flow="no-wrap" label={(<div>
            {translate('step')}
...

        {
          type === 'navButtonGroup' && <React.Fragment>
            <SettingRow flow="wrap" label={translate('previous')} className="justify-content-between">
              <TextInput size="sm" style={{ width: '61%' }} value={newPreviousText} onChange={handlePreviousTextChange} onAcceptValue={(value) => onSettingConfigChange(['display', 'standard', 'previousText'], value)} />
              <IconPicker configurableOption={'none'} icon={previousIcon as any} onChange={(icon) => onSettingConfigChange(['display', 'standard', 'previousIcon'], icon)} setButtonUseColor={false}></IconPicker>
            </SettingRow>

            <SettingRow flow="wrap" label={translate('next')} className="justify-content-between">
              <TextInput size="sm" style={{ width: '61%' }} value={newNextText} onChange={handleNextTextChange} onAcceptValue={(value) => onSettingConfigChange(['display', 'standard', 'nextText'], value)} />
              <IconPicker configurableOption={'none'} icon={nextIcon as any} onChange={(icon) => onSettingConfigChange(['display', 'standard', 'nextIcon'], icon)} setButtonUseColor={false}></IconPicker>
            </SettingRow>
    ...
    {section && <SettingSection>
      ...
      {advanced && <React.Fragment>
        ...
        {type === 'nav' && !showIcon && <SettingRow label={translate('tabStyle')} flow="no-wrap"></SettingRow>}
        {type === 'navButtonGroup' && <SettingRow label={translate('navBtnStyle')} flow="no-wrap"></SettingRow>}
