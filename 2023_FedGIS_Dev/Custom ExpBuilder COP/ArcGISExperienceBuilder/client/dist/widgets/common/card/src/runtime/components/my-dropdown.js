...
export default class MyDropDown extends React.PureComponent {
    ...
    render() {
        const { items, toggleContent, caret, toggleType, menuContent, modifiers, toggleIsIcon, theme, isDropDownOpen, size, appendToBody, toggleTitle, direction, menuCss, className, showActive, activeLabel, fluid } = this.props;
        ...
        return (jsx(MyDropdown, { size: size, toggle: this.onDropDownToggle, direction: direction, fluid: fluid, isOpen: isOpen, className: `my-dropdown ${className}`, css: this.getStyle() },
            toggleContent && (jsx(MyDropdownButton, { icon: toggleIsIcon, title: toggleTitle, size: size, type: toggleType, caret: caret, arrow: false, className: 'card-toggle-button' }, toggleContent && toggleContent(theme))),
            jsx(MyDropdownMenu, { appendToBody: appendToBody, modifiers: modifiers, css: menuCss && menuCss(theme) }, menuContent
                ...
                : items &&
                    items.asMutable().map((item, i) => {
                        return (!item.hide && (jsx(MyDropdownItem, { key: i, className: 'no-user-select', active: showActive && item.label === activeLabel, onClick: evt => this.onItemClick(evt, item) }, item.label)));
                    }))));
    }
}
