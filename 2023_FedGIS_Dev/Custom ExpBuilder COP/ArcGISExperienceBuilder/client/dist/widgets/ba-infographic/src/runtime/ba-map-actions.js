...
export default class BAMapActions {
    /** constructor
       ...
       * }
       */
    constructor(widgetId, mapId, showSearch, reportChangeCallback, context) {
        this.ignorePropChanges = false;
        this._allowPropChanges = this._debounce(function () {
            this.ignorePropChanges = false;
        }, 1000, false);
    ...
    updateMap() {
        if (!this.ignorePropChanges) {
            ...
            D.log('map-actions', 'updateMap ignored prop change');
    }
    ...
    syncSettings() {
        ...
        const values = [];
        const sArr = this.context.bufferSizes.split(',');
        sArr.forEach(v => {
    ...
    initialize(view) {
        ...
        if (self.showSearch) {
            self.searchControl = MapSearch.registerCallbacks(self.widgetId, self.mapWidgetId, self.view, function onReceivedSearchResults(results, bamInstance) {
        ...
        self.view.on('click', evt => {
            ...
            function highlightPoint(response, data) {
            ...
            function highlightPolygon(response, data) {
    ...
    render(lat, lon, noClear) {
        ...
        if (elem) {
            if (this.bufferType === 'ring') {
                ...
                // @ts-expect-error
                elem.generateBuffers(this.bufferType, this.bufferSizesString, // "1,3,5"
                this.units, // 'minutes', 'miles', or 'kilometers'
                lat.toString(), lon.toString()).then(function (data) {
                    if (!data || data.error) {
...
//# sourceMappingURL=ba-map-actions.js.map